...
export class ElementFindService {
  ...
  private _getByElementCollection(query: ElementCollectionModel): SourceElementModel {
    ...
    if (query.id !== ElementCollection.Root) {
      const elem = this.contextElement
        .querySelector(".viewport__content-section .modal-body .panel-group")
        ?.parentNode?.parentNode?.querySelector(".control-label") as HTMLElement;
      if (elem) {
        return elem;
      }
    }
